...
1. **Overcoming RAM Exhaustion (Ingestion):** Currently, the `IndexFlatIP` Faiss backend accumulates vectors in system RAM before writing to disk. At 10+ million chunks, this would trigger an Out-Of-Memory (OOM) crash. To scale, the pipeline could be upgraded to use `OnDiskInvertedLists` or Index Sharding, allowing vectors to stream directly to NVMe SSDs during the `add()` operation.
